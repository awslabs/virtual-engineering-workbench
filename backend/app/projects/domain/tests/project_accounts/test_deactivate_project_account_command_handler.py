...
from app.projects.domain.command_handlers.project_accounts import deactivate_project_account_command_handler
from app.projects.domain.commands.project_accounts import deactivate_project_account_command
...
def test_can_deactivate_active_project_account(handler_dependencies, mock_uow_2, mock_account_repo):
    # Arrange
    account = make_fake_project_account(status=project_account.ProjectAccountStatusEnum.Active)
    command = deactivate_project_account_command.DeactivateProjectAccountCommand(
        account_id=account_id_value_object.from_str(account.id),
        account_status=account_status_value_object.from_value_str("Inactive"),
    ...
    # Act
    deactivate_project_account_command_handler.handle_deactivate_project_account_command(
    ...
    assertpy.assert_that(ent.accountStatus).is_equal_to("Inactive")
