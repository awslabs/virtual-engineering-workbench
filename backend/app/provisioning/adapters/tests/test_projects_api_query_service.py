...
from app.provisioning.adapters.query_services import projects_api_query_service
...
def test_get_aws_account_by_id_returns_correct_account(mock_api):
    ...
    mock_api.call_api.assert_called_once_with(
        ...
        http_method="GET",
        query_params={"pageSize": "20", "projectId": "proj-id-1"},
    )
...
def test_get_user_assignments_count_return_assignments_count(mock_api):
    ...
    mock_api.call_api.assert_called_once_with(
        ...
        http_method="GET",
        query_params={"pageSize": "20", "userId": "user-1"},
    )
...
@mock.patch("app.shared.api.aws_api.AWSAPI")
def test_get_project_assignment_should_return_an_assignment(mock_api):
    # ARRANGE
    project_api_qry_srv = projects_api_query_service.ProjectsApiQueryService(api=mock_api)
    mock_api.call_api.return_value = {
        "assignment": {
    ...
    # ACT
    assignment = project_api_qry_srv.get_project_assignment("project-id", "user-id")

    # ASSERT
    mock_api.call_api.assert_called_once_with(
        path="internal/projects/project-id/users/user-id",
        http_method="GET",
    )
    assertpy.assert_that(assignment).is_not_none()


@mock.patch("app.shared.api.aws_api.AWSAPI")
def test_get_project_assignment_should_return_none_when_no_assignment(mock_api):
    # ARRANGE
    project_api_qry_srv = projects_api_query_service.ProjectsApiQueryService(api=mock_api)
    mock_api.call_api.return_value = {"assignment": None}

    # ACT
    assignment = project_api_qry_srv.get_project_assignment("project-id", "user-id")

    # ASSERT
    assertpy.assert_that(assignment).is_none()


@mock.patch("app.shared.api.aws_api.AWSAPI")
def test_get_project_returns_correct_project(mock_api):
    # ARRANGE
    project_api_qry_srv = projects_api_query_service.ProjectsApiQueryService(api=mock_api)
    mock_api.call_api.return_value = {
        "projects": [
