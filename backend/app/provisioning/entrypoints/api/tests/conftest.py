...
def disable_migrations():
    with mock.patch("app.provisioning.entrypoints.api.bootstrapper.migrations_config", return_value=[]):
        yield
...
def aws_credentials(monkeypatch):
    ...
    monkeypatch.setenv(
        "APPLICATION_VERSION_FRONTEND_PARAMETER_NAME", "/proserve-wb-provisioning-api/application_version_frontend"
    )
    monkeypatch.setenv(
        "APPLICATION_VERSION_BACKEND_PARAMETER_NAME", "/proserve-wb-provisioning-api/application_version_backend"
    )
    monkeypatch.setenv(
...
def api_schema():
    ...
    schema_path = os.path.join(
        ...
        "..",
        "schema",
        "proserve-workbench-provisioning-api-schema.yaml",
    )
