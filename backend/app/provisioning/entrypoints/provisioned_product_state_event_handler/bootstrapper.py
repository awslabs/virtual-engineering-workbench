...
from app.provisioning.domain.command_handlers.provisioned_product_state import complete_start, complete_stop
...
from app.provisioning.entrypoints.provisioned_product_state_event_handler import config
...
def bootstrap(  # noqa: C901
    ...
    def _get_sm_boto_client(aws_account_id: str, region: str, user_id: str) -> ssm_client.SSMClient:
        return _get_boto_client_for(
            client_name="secretsmanager", aws_account_id=aws_account_id, region=region, user_id=user_id
        )
    ...
    parameter_srv = aws_parameter_service.AWSParameterService(
        ssm_boto_client_provider=_get_ssm_boto_client, sm_boto_client_provider=_get_sm_boto_client
    )

    container_mgmt_srv = ecs_container_management_service.ECSContainerManagementService(
        ecs_boto_client_provider=_get_ecs_boto_client,
        logger=logger,
    )

    subnet_selector = networking_helpers.get_provisioning_subnet_selector(
        app_config.get_provisioning_subnet_selector(), tag=app_config.get_provisioning_subnet_selector_tag()
    )

    def _complete_start_handler_factory():
        def _handle(command):
    ...
    def _complete_stop_handler_factory():
        def _handle(command):
    ...
    def _update_provisioned_product_handler_factory():
        def _handle(command):
    ...
    return Dependencies(
        ...
        instance_mgmt_srv=instance_mgmt_srv,
        container_mgmt_srv=container_mgmt_srv,
        provisioned_products_query_service=provisioned_products_qs,
    )
