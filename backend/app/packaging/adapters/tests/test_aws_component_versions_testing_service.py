...
def test_should_get_testing_environment_image_upstream_id(
    ...
    # ARRANGE & ACT
    image_upstream_id = mock_aws_component_version_testing_service.get_testing_environment_image_upstream_id(
    ...
    # ASSERT
    assertpy.assert_that(image_upstream_id).is_equal_to(GlobalVariables.TEST_AMI_ID.value)


def test_should_get_testing_environment_instance_type(
    mock_aws_component_version_testing_service, mock_security_group, mock_subnets
):
    # ARRANGE & ACT
    instance_type = mock_aws_component_version_testing_service.get_testing_environment_instance_type(
        architecture=GlobalVariables.TEST_ARCHITECTURE.value,
        platform=GlobalVariables.TEST_PLATFORM.value,
        os_version=GlobalVariables.TEST_OS_VERSION.value,
    )
...
def test_should_launch_testing_environment(
    mock_aws_component_version_testing_service, mock_instance_profile, mock_security_group, mock_subnets
):
    # ARRANGE & ACT
    instance_id = mock_aws_component_version_testing_service.launch_testing_environment(
        image_upstream_id=GlobalVariables.TEST_AMI_ID.value, instance_type=GlobalVariables.TEST_INSTANCE_TYPE.value
    )
...
def test_launch_should_retry_all_subnets_on_retryable_error(
    ...
    mock_aws_component_version_testing_service = mock_aws_component_version_testing_service_factory(
...
def test_launch_should_raise_when_subnets_exhausted(
    ...
    with pytest.raises(adapter_exception.AdapterException) as e:
        mock_aws_component_version_testing_service.launch_testing_environment(
            image_upstream_id=GlobalVariables.TEST_AMI_ID.value, instance_type=GlobalVariables.TEST_INSTANCE_TYPE.value
        )
...
def test_should_raise_an_exception_with_no_subnets_available(
    mock_aws_component_version_testing_service, mock_vpc, mock_security_group
):
    # ARRANGE & ACT
...
def test_should_raise_an_exception_with_no_security_groups_available(
    mock_aws_component_version_testing_service, mock_subnets
):
    # ARRANGE & ACT
    with pytest.raises(adapter_exception.AdapterException) as exec_info:
        mock_aws_component_version_testing_service.launch_testing_environment(
...
def test_should_get_testing_environment_creation_time(
    mock_aws_component_version_testing_service, mock_instance_profile, mock_ec2_instance
):
    # ARRANGE
    instance_id = mock_ec2_instance.get("Instances")[0].get("InstanceId")

    # ACT
    response = mock_aws_component_version_testing_service.get_testing_environment_creation_time(instance_id=instance_id)

    # ASSERT
    assertpy.assert_that(response).is_equal_to("2023-10-13 00:00:00")


def test_should_get_testing_environment_status(mock_aws_component_version_testing_service, mock_moto_calls):
    # ARRANGE & ACT
    response = mock_aws_component_version_testing_service.get_testing_environment_status(
...
def test_should_setup_testing_environment(
    mock_aws_component_version_testing_service, mock_ssm_client, mock_system_configuration_mapping
):
    # ARRANGE & ACT
    command_id = mock_aws_component_version_testing_service.setup_testing_environment(
        architecture=GlobalVariables.TEST_ARCHITECTURE.value,
        instance_id=GlobalVariables.TEST_INSTANCE_ID.value,
        os_version=GlobalVariables.TEST_OS_VERSION.value,
        platform=GlobalVariables.TEST_PLATFORM.value,
...
def test_should_get_testing_command_status(
    mock_aws_component_version_testing_service, mock_ssm_client, mock_command_id
):
    # ARRANGE & ACT
...
def test_should_run_testing(
    ...
    # ARRANGE
    component_version_definition_s3_uri = "s3://fake_bucket/component_definition.yaml"

    # ACT
    command_id = mock_aws_component_version_testing_service.run_testing(
        architecture=GlobalVariables.TEST_ARCHITECTURE.value,
        component_version_definition_s3_uri=component_version_definition_s3_uri,
        instance_id=GlobalVariables.TEST_INSTANCE_ID.value,
        os_version=GlobalVariables.TEST_OS_VERSION.value,
        platform=GlobalVariables.TEST_PLATFORM.value,
        component_id=GlobalVariables.TEST_COMPONENT_ID,
        component_version_id=GlobalVariables.TEST_COMPONENT_VERSION_ID,
    )
    response = mock_ssm_client.get_command_invocation(
        CommandId=command_id,
        InstanceId=GlobalVariables.TEST_INSTANCE_ID.value,
...
def test_should_teardown_testing_environment(
    mock_aws_component_version_testing_service, mock_ec2_client, mock_instance_profile, mock_ec2_instance
):
    # ARRANGE
    instance_id = mock_ec2_instance.get("Instances")[0].get("InstanceId")

    # ACT
    response = mock_aws_component_version_testing_service.teardown_testing_environment(instance_id=instance_id)
